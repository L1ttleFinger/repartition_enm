...
def verification_voeux(voeux_df, postes_df, params_dict):
    ...
    for index, row in voeux_df.iterrows():
        ...
        if len(voeux_aud) < voeux_max:
            ...
            print("Auditeur " + aud + ":\tERREUR ! Voeu(x) inconnu(s) :\t", inconnues)
            voeux_df.iloc[index, 1:] = np.nan
            erreurs += 1
        elif not (verifier_unicite_voeux(voeux_aud)):
            print("Auditeur " + aud + ":\tERREUR ! Doublons :\t", voeux_aud)
            voeux_df.iloc[index, 1:] = np.nan
            erreurs += 1
        elif (
            (n_noir > noires_max)
            ...
            print(
                "Auditeur " + aud + ":\tERREUR ! Couleurs (" + str(n_noir) + "N, " + str(n_rouge) + "R, " + str(n_vert) + "V)\t"
            )
            voeux_df.iloc[index, 1:] = np.nan
            erreurs += 1
        else:
...
def distribution_des_voeux(villes, voeux_df, postes_df, voeux_max):
    ...
    for ville in postes_df[postes_df['total_demandes'] == 0]['Ville']:
        ...
        print("Aucun auditeur n'a placé " + ville + " dans ses voeux (" + str(capacite) + " places)")
        nb_postes_non_demandes = nb_postes_non_demandes + capacite
    
    print(
        "Il y a " + str(nb_postes_non_demandes) + " postes qui n'ont été demandés par personne."
    )
...
def creer_matrice_couts(nb_auditeurs, nb_postes, repartition_df, villes, params_dict, penalite):
    voeux_max = params_dict['Voeux max']
    # lignes : nb_auditeurs ; colonnes : nb_postes
    graph = np.full((nb_auditeurs, nb_postes), penalite)
    ...
    for ligne, row in repartition_df.iterrows():
        ...
        for num_voeu in range(len(voeux_aud)):
            ...
            for col in colonnes:
                cout = voeu_vers_cout(num_voeu, params_dict['Methode'])
                if num_voeu > (voeux_max - 1):
                    cout *= 10
                graph[ligne, col] = cout
    return graph
